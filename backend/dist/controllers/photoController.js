...
const uploadMiddleware = (req, res, next) => {
    res.status(410).json({
        success: false,
        error: "This upload method is deprecated. Please use the new direct B2 upload system.",
    });
};
...
const downloadPhoto = async (req, res) => {
    try {
        ...
        // Check if gallery has expired
        if (photo.folder.gallery.expiresAt &&
        ...
        if (photo.folder.gallery.password) {
            ...
            const token = bodyToken || queryToken || (authHeader && authHeader.split(" ")[1]);
            if (token) {
                try {
                    const jwt = await Promise.resolve().then(() => tslib_1.__importStar(require("jsonwebtoken")));
                    const decoded = jwt.default.verify(token, process.env.JWT_SECRET);
...
const createDownloadTicket = async (req, res) => {
    try {
        ...
        const ticket = jsonwebtoken_1.default.sign(ticketPayload, process.env.JWT_SECRET, { expiresIn: '5m' }); // Ticket is valid for 5 minutes
...
const downloadWithTicket = async (req, res) => {
    try {
        const { ticket } = req.query;
        if (!ticket) {
            return res.status(400).json({ success: false, error: "Missing download ticket" });
        }
        const decoded = jsonwebtoken_1.default.verify(ticket, process.env.JWT_SECRET);
        ...
        switch (filter) {
            ...
            case 'liked':
            case 'favorited':
                await downloadService_1.DownloadService.createFilteredPhotoZip(galleryId, userId, filter, res);
                break;
            default:
...
exports.downloadWithTicket = downloadWithTicket;
